...
int main(int argc, char** argv) {
	std::cout << std::endl << "------------ Z-test simulated dataset graph ------------" << std::endl;
    ...
    for (int i = 0; i < total_changes; i++) {
		...
		output_z_dec_test[i] =  (output_dec_mean - population_mean) / (population_sd / sqrt(num_records)); 
		//cout << "output z-test: " << output_z_dec_test[i] << ", output z-test precision: " << output_z_dec_test[i] << endl;
    }
