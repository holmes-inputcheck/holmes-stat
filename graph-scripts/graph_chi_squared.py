...
def find_unnormalized_p_value(num_records):
    ...
    f = open("chi_test_p_graph.csv", "w")
    first = True
    f.write("EntriesChanged,NormalPvalue,UnnormalPvalue,JLPvalue\n")
    ...
    # Number of features in simulated dimensions
    D = size_of_each_dimension ** num_dimensions
    ...
    for i in range(len(list_jlchi)):
        actual_p_value = scipy.stats.chi2.sf(list_actualchi[i], df=D - 1)
...
subprocess.run(["bin/graph_chi_squared"])
