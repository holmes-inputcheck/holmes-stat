...
def copy_p_value_to_csv(num_records):
    ...
    res = f.readlines()
    f.close()
    f = open("f_test_p_graph_dataset1.csv", "w")
    first = True
    ...
    for line in res:
        ...
        p_value = (1-scipy.stats.f.cdf(abs(float(sep[-1])), (num_records // 2) - 1, num_records - 1)) * 2
...
subprocess.run(["bin/graph_f_test_dataset1"])
